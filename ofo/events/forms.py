...
class NewEventForm(forms.Form):
    ...
    duration = forms.IntegerField(label='Dauer (Minuten)', initial=30, min_value=15, widget=forms.NumberInput(
        attrs={'class': 'form-control'}))
    date = forms.DateField(label='Datum', initial=datetime.now().strftime("%Y-%m-%d"), widget=forms.widgets.DateInput(
        attrs={'class': 'form-control', 'type': 'date'}))
    time = forms.TimeField(label='Uhrzeit', initial=datetime.now().strftime("%H:%M"), widget=forms.widgets.TimeInput(
        attrs={'class': 'form-control', 'type': 'time'}))

    def clean_date(self):
        """clean_date is called when form.is_valid() is called. It is tested if the date is in the future"""
...
class ProposalForm(forms.Form):
    """Form for giving a proposal to a specific event"""
    ...
    place = forms.CharField(label='Ort vorschlagen', max_length=50, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': 'Vorschlag Ort'}))
    comment = forms.CharField(label='Kommentar', max_length=200, widget=forms.widgets.Textarea(
        attrs={'class': 'form-control', 'placeholder': 'Kommentar eingeben..', 'rows': '3'}))


class DetermineEventForm(forms.Form):
    """Form for finally determing the event by the organisator"""
    ...
    time = forms.TimeField(label='Uhrzeit festlegen', initial=datetime.now().strftime("%H:%M"),
                           widget=forms.widgets.TimeInput(attrs={'class': 'form-control', 'type': 'time'}))
    place = forms.CharField(label='Ort festlegen', max_length=50, widget=forms.TextInput(
